...
As it will be discussed more in this and subsequent articles, the key that unlocks the vDC is a centralized topology (hub and spokes) with a mix of Azure features: Azure VNet, NSG, UDR, VNet peering, and Azure Identity with Role Base Access Control.
...
Following an approach top-down enterprise business create a demanding of services for the different LOBs, and employees  with different company roles are involved with different responsibility in deployments and maintenance of projects.
...
Azure Site-to-Site VPN can be deployed in multi-sites configuration, with vDC connected with multiple on-premises networks. The configuration is also extremely useful in all cases where  customer branch offices do not have a private interconnection with the head quarter and want to be connected to the vDC.
**ExpressRoute** is an Azure connectivity service that lets you create **private connections** between vDC and the on-premises networks. ExpressRoute connections do not go over the public Internet, and offer higher security, reliability and speeds with lower latencies compare with connections over the Internet. ExpressRoute is a **must** to have of every large vDC, due to the benefit of integration with on-premises networks and the satisfaction of most restrictive rules of security of compliance, associated with private connections.
The deployment of ExpressRoute connection requires an agreement with ExpressRoute service provider and it is a longer process than Site-to-Site VPN. For customer that need to start quickly with lift-and-shift approach, it is quite common to make leverage on Site-to-Site VPN to ram up fast and review the connectivity later, when the ExpressRoute connection will be completed successful.
...
The high level digram of vDC shows the different component types in the different zones and the hub-spokes topology. The diagram reveals the presence of infrastructure in different parts of architecture. 


As good practice (for on-premises DC or vDC) the rights are set on groups; this allows consistency set of right across the team and avoid reconfigurations due to change of role in the organization or leaving the company.
...
In software deployment, an environment (or tier) is a  system in which multiple applications are deployed and executed. The large enterprise use the development environment (where changes are originally made) and production environment (what end users use) are separated, often with several stages in between, in order to allow phased deployment (rollout), testing, and rollback in case of problems. Deployment architectures vary significantly, but broadly, the tiers are bookended by starting at development (DEV) and ending at production (PROD). 
A common tier architecture is devops (development and testing), UAT (staging), production.
Organization can make leverage on single or multiple Azure AD tenants. The diagram shows the case where two different Azure AD tenants are used: one for devops and UAT and the other exclusive for production. The presence of different Azure AD tenant enforce the separation between environments. The same group of users (e.g. Central IT) need to authenticate with different URI to different AD tenant to get access to devop or production environments of a specific project. A different change in user login to access to different environments, reduce the outage caused by human errors.

There are few simple cases can be discussed to understand the implications:

- every environment (e.g. devops) can share the same Azure subscription between different projects of the same LOB. The approach help to limit is reduce the number of Azure subscriptions. The inconvenient of this approach is the presence of large numbers of Resource Groups in presence of many different projects; in this case is recommended a technique to lock the Resource Groups to avoid accidental deletions.   
- every environment (e.g. UAT) of a project is created in specific Azure subscriptions. there is no sharing of Azure subscription between different project. This approach have strong separation of environments , but it potentially prone to an excess of proliferation of subscriptions in vDC. (e.g. in company with 5 LOBs and 4 projects, each with three different environments, requires 5*4*3= subscriptions).
...
- **UDR**. User Define Route is a custom routing table that network administrator can associate to one or more subnets to overwrite the behaviour of system table behaviour. If UDR is an optional component in some specific scenarios, it becomes a fundamental component in the vDC. The presence of UDRs guarantee that egress traffics from the spoke transit through Network Virtual Appliances  and load balancer present in the hub and in the spokes.
- **NSG**. In Azure the TCP/UDP flows control is nativity implemented through Network Security Group (NSG). A Network Security Group is a list of security rules that act as traffic filtering; each rule is based on a tuple of 5 values: IP Source, IP Destination, Protocol, IP Source Port and IP Destination port. One powerful expect of NSG that the filtering mechanism is stateful: when a rule is created to allow to establish the flow, the flow related to the answer is automatically accepted. This property of NSG help to reduce the list of entries in the security policy. The NSG can be applied to a subnet or a Virtual NIC card of an Azure VM or both. To have an agile management of security policies in vDC is recommended to make leverage of NSGs applied to subnets: this avoid a proliferation of NSGs with possibility to reuse the same NSG for different deployments (dev and test, UART, pre-production and production). The NSGs are essential to implement a correct flows control in the hub and in the spokes.
- **DNS**. If not specified, the VNet uses an default Azure DNS (assigned automatically from Network Resource Provider) to resolve naming inside the VNets. The scope of default DNS is limited to the VNet. Name resolution is therefore not extended to  VNet in peering neither VNet-to-VNet. The default DNS associate to the VNets normally doesn't fit with customer requirements. In most of cases a custom DNS service need to be deployed in the hub as  part of common services. The main consumers of DNS reside in the spoke. As example of resolution name required for a correct implementation of the workload is the case of a two tiers application with a web front that need to communicate with a SQL server in backed by ALIAS.
...
One other challenge of multi vDC architecture is the synchronization between workloads located in different vDC. Ideally customers want to synchronize application data  between deployments running in two different vDC,to implement a fast failover mechanism between vDC. Most of applications are sensitive to latency in transmission, that cause potential timeout and delay in data synchronization. Alignment of data in synch mode is a challenge also when the data churn is high and forbidden the commitment of transactions on remote vDC. 
...
In most of cases the Expressroute connection is the preferred  mechanism due higher bandwidth and low latency with transit through the Microsoft backbone. 
